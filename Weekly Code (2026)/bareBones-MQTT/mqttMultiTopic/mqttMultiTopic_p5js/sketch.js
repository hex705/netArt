...
// MQTT - shiftr 
// PUBLISH TOPIC
let PUBlishTopicRed   = "p5_pubRed";
let PUBlishTopicGreen = "p5_pubGreen";
let PUBlishTopicBlue  = "p5_pubBlue";

let payload = "empty";

//SUBscribe topic
let SUBscribeTopicRed   = "ard_pubRed"; 
let SUBscribeTopicGreen = "ard_pubGreen"; 
let SUBscribeTopicBlue  = "ard_pubBlue"; 
...
// feedbackValues
let subscribeRed=0, subscribeGreen=0, subscribeBlue=0;
let subscribeColor;
let incomingPayload; 
...
function drawScreen(){
  ...
  // draw feedback color 
  subscribeColor = color(subscribeRed, subscribeGreen, subscribeBlue);
  fill(subscribeColor);
  ...
  fill(87,87,87);
  textSize(16);
  text('color from arduino:: '+ subscribeRed+","+subscribeGreen+","+subscribeBlue,350, 475);
...
function buildColorMessage(){
  ...
  if ( isNaN(r) || isNaN(g) || isNaN(b)){
    ...
    assembleMessage(PUBlishTopicRed, r);
    assembleMessage(PUBlishTopicGreen,g);
    assembleMessage(PUBlishTopicBlue, b);
  }
  
}

// use glue to build our message 
function assembleMessage (sendTopic, sendValue){
  ...
  publishMqttMessage(sendTopic, payload);
